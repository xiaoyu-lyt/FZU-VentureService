	...
	<div class="admin-new-table admin-info-table admin-table block"> <!-- 注意这个block -->
		<table>
			<tr>
				...
				<th class="admin-th-article-publisher admin-th-publisher" style="width:90px"><span>发布人</span></th>
				<th class="admin-th-article-hot admin-th-hot" style="width:75px"><span>是否热门</span></th>
		...
		<?php else: ?>
			<?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
					<td class="admin-article-select">
						<input id="<?php echo ($v["nid"]); ?>" class="admin-news-select-btn admin-select-btn" type="checkbox">
					</td>
					...
					<td class="admin-th-article-hot admin-th-hot"><span><?php if($v["overhead"] == 1): ?>是<?php else: ?>否<?php endif; ?></span></td>
					...
					<td class="admin-article-operation admin-operation">
						...
						<span class="admin-article-delete"><a onclick="javascript: if(confirm('确定删除？')) return true; return false;" href="/FZU-VentureService/Admin/index.php/home/notice/deleteOne?nid=<?php echo ($v["nid"]); ?>">删除</a></span>
					</td>
...
<!-- 分页 -->
	<div class="pull-right">
		<ul class="pagination">
			...
			<?php if(($curPage > 3) AND ($curPage < $totalPage-2)): $__FOR_START_21665__=$curPage-2;$__FOR_END_21665__=$curPage+3;for($i=$__FOR_START_21665__;$i < $__FOR_END_21665__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/FZU-VentureService/Admin/index.php/home/notice/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } ?>
			<?php elseif(($curPage > $totalPage-3) AND ($totalPage > 5)): ?>
				<?php $__FOR_START_18118__=$totalPage-5;$__FOR_END_18118__=$totalPage;for($i=$__FOR_START_18118__;$i < $__FOR_END_18118__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/FZU-VentureService/Admin/index.php/home/notice/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } ?>
			<?php elseif($totalPage > 5): ?>
				<?php $__FOR_START_9863__=1;$__FOR_END_9863__=6;for($i=$__FOR_START_9863__;$i < $__FOR_END_9863__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/FZU-VentureService/Admin/index.php/home/notice/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } ?>
			<?php else: ?>
				<?php $__FOR_START_14772__=1;$__FOR_END_14772__=$totalPage;for($i=$__FOR_START_14772__;$i < $__FOR_END_14772__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/FZU-VentureService/Admin/index.php/home/notice/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } endif; ?>
