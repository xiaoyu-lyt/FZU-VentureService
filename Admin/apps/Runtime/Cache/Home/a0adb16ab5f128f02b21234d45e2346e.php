...
<body>
	...
	<div class="user-box block">
		<form class="publish-form base-publish form-horizontal" enctype="multipart/form-data" action="/FZU-VentureService/Admin/index.php/home/field/modify/<?php echo ($detail["fid"]); ?>/do" method="post">
			<h2 align="center" style="margin: 0 0 20px 0; color: #61a3e1;">创业基地发布</h2>
		 	<table class="publish-table">
				<tr>
					...
					<td><input name="name" style="width: 280px;" class="title-input form-control" type="text" value="<?php echo ($detail["name"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>法定代表人：</h4></td>
					<td><input name="chief" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["chief"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>基地地址：</h4></td>
					<td><input name="addr" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["addr"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>基地面积：</h4></td>
					<td><input name="area" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["area"]); ?>">
					</td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>产权单位：</h4></td>
					<td><input name="owner" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["owner"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>自有/合办：</h4></td>
					<td>
						<label for="own" class="radio-inline">
						<input id="own" name="own_or_co"  type="radio" value="1" <?php if($detail["own_or_co"] == 1): ?>checked<?php endif; ?> >自有
						</label>
						<label for="co" class="radio-inline">
						<input id="co" name="own_or_co"  type="radio" value="2" <?php if($detail["own_or_co"] == 2): ?>checked<?php endif; ?> >合办
						</label>
				...
				<tr>
					...
					<td><input name="investment_field" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["investment_field"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>其中学校投入：</h4></td>
					<td><input name="investment_class" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["investment_class"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>其他投入：</h4></td>
					<td><input name="investment_other" style="width:280px;" class="title-input form-control" type="text" value="<?php echo ($detail["investment_other"]); ?>"></td>
				</tr>
				<tr>
					<td><h4 class="line-title text-right"><span class="star">*</span>基地简介：</h4></td>
					<td>
						<textarea class="form-control" style="width: 280px; height: 100px;" name="synopsis" id="" cols="30" rows="10"><?php echo ($detail["synopsis"]); ?></textarea>
				...
				<tr>
					...
					<!-- 下面插入富文本 -->
					<td><script id="_container" name="detail" type="text/plain" style="width: 100%; height:300px;"><?php echo ($detail["detail"]); ?></script>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<img width="300" height="200" src="<?php echo ($detail["pic"]); ?>" /><br><br>
				...
				<tr>
					...
					<td>
						...
						<div class="input-group date form_date col-md-4" data-date="" data-date-format="yyyy-mm-dd" data-link-field="run_time" data-link-format="yyyy-mm-dd">
		                    <input  class="form-control" size="16" type="text" placeholder="点击选择日期" value="<?php echo ($detail["run_time"]); ?>" readonly>
		                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
							<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
		                </div>
		                <input type="hidden" id="run_time" name="run_time" value="<?php echo ($detail["run_time"]); ?>" ><br/>
		                <!-- 日期插件结束 -->
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<input id="leisure" name="status"  type="radio" value="1" <?php if($detail["status"] == 1): ?>checked<?php endif; ?> >是
						</label>
						<label for="occupies" class="radio-inline">
						<input id="occupies" name="status"  type="radio" value="0" <?php if($detail["status"] == 0): ?>checked<?php endif; ?> >否
						</label>
				...
				<?php if(is_array($detail["public"])): $i = 0; $__LIST__ = $detail["public"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><input name="public[<?php echo ($i); ?>][name]" style="width: 280px;" class="title-input form-control" type="text" value="<?php echo ($vo["name"]); ?>"></td>
					</tr>
					<tr>
						<td><h4 class="line-title text-right"><span class="star">*</span>公告场地说明：</h4></td>
						<td>
							<textarea class="form-control" style="width: 280px; height: 100px;" name="public[<?php echo ($i); ?>][synopsis]" id="" cols="30" rows="10"><?php echo ($vo["synopsis"]); ?></textarea>
							<!-- <input name="synopsis" style="width:280px;" class="title-input form-control" type="text"> -->
						</td>
					</tr>
					<tr>
						<td><h4 class="line-title text-right"><span class="star">*</span>基地照片：</h4></td>
						<td>
							<img width="300" height="200" src="<?php echo ($vo["pic"]); ?>" /><br><br>
							<input type="hidden" name="public[<?php echo ($i); ?>][pic_pre]" value="<?php echo ($vo["pic_name"]); ?>"></intpu>
							<input name="pic_<?php echo ($i); ?>" type="file" >
						</td>
