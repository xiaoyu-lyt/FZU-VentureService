...
<div class="user-box admin-users-management sub-management pull-right">
			...
			<?php else: ?>
				<?php if(is_array($investors)): $i = 0; $__LIST__ = $investors;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
						...
						<td class="admin-teacher-operation admin-operation">
							...
							<a onclick="javascript: if(confirm('确定删除？')) return true; return false;" href="/demo/jyzd/01/Admin/index.php/home/user/delete?uid=<?php echo ($v["uid"]); ?>&module=investor"><span class="admin-view-info">删除</span></a>
						</td>
	...
	<div class="pull-right">
		<ul class="pagination">
			...
			<?php if(($curPage > 3) AND ($curPage < $totalPage-2)): $__FOR_START_20428__=$curPage-2;$__FOR_END_20428__=$curPage+3;for($i=$__FOR_START_20428__;$i < $__FOR_END_20428__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/demo/jyzd/01/Admin/index.php/home/user/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } ?>
			<?php elseif(($curPage > $totalPage-3) AND ($totalPage > 5)): ?>
				<?php $__FOR_START_21013__=$totalPage-5;$__FOR_END_21013__=$totalPage;for($i=$__FOR_START_21013__;$i < $__FOR_END_21013__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/demo/jyzd/01/Admin/index.php/home/user/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } ?>
			<?php elseif($totalPage > 5): ?>
				<?php $__FOR_START_2346__=1;$__FOR_END_2346__=6;for($i=$__FOR_START_2346__;$i < $__FOR_END_2346__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/demo/jyzd/01/Admin/index.php/home/user/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } ?>
			<?php else: ?>
				<?php $__FOR_START_25371__=1;$__FOR_END_25371__=$totalPage;for($i=$__FOR_START_25371__;$i < $__FOR_END_25371__;$i+=1){ ?><li><a <?php if($i==$curPage) echo "class='now'"; ?> href="/demo/jyzd/01/Admin/index.php/home/user/<?php echo ($now); ?>/<?php echo ($i); ?>" ><?php echo ($i); ?></a></li><?php } endif; ?>
