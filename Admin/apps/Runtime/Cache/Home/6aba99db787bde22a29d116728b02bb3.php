...
<div class="article-publish-form">
	<form action="/demo/jyzd/01/Admin/index.php/home/notice/publish/do" method="post">
		<div class="article-publish-box article-publish-column">
			<label for=""><span class="star">*</span>栏目:</label>
			<select name="type" >
				<option value="0" <?php if($article['type'] == 0 ) echo "selected='selected'";?> >新闻热点</option>
				<option value="1" <?php if($article['type'] == 1 ) echo "selected='selected'";?> >通知公告</option>
				<option value="2" <?php if($article['type'] == 2) echo "selected='selected'";?> >最新政策</option>
			</select>
		</div>
		<div class="article-publish-box article-publish-title">
			<label for=""><span class="star">*</span>标题:</label>
			<input type="text" name="theme" value="<?php echo ($article["theme"]); ?>">
